...
${CMD_PREFIX} " umask ${UMASK}; \"${JAVA}\" ${JAVA_OPTS} \
  \"-Dlogging.configuration=file:${JBOSS_BASE_DIR}/configuration/jgroups-probe-logging.properties\" \"-Djboss.jgroups-probe.log.file=${JBOSS_BASE_DIR}/log/jgroups-probe.log\" \
  -jar \"${JBOSS_HOME}/jboss-modules.jar\" -mp \"${JBOSS_MODULEPATH}\" org.jgroups.probe \
  ${PARAM} "
