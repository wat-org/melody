...
public class IngressMachine extends AbstractLibVirtOperation implements ITask {
	...
	public void doProcessing() throws LibVirtException, InterruptedException {
		...
		FwRulesDecomposed rl = getInstanceFireWallRules(i);
		FwRulesDecomposed toAdd = null;
		FwRulesDecomposed toRemove = null;
		toAdd = FireWallRulesHelper.computeFireWallRulesToAdd(rl, getFwRules());
		toRemove = FireWallRulesHelper.computeFireWallRulesToRemove(rl,
				getFwRules());

		log.info(Messages.bind(Messages.IngressMsg_FWRULES_RESUME,
				new Object[] { getInstanceID(), getFwRules(), toAdd, toRemove,
						getTargetNodeLocation() }));

		revokeFireWallRules(i, toRemove);
		authorizeFireWallRules(i, toAdd);
